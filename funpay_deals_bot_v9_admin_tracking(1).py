...
DATA_BACKUP_PATHS = [DATA_PATH.with_name(f"{DATA_PATH.name}.bak{index}") for index in range(1, 4)]
LOG_PATH = Path(os.getenv("BOT_LOG_PATH", str(Path(__file__).with_name("bot.log"))))
...
# после каждого изменения. Неизвестные поля верхнего уровня тоже сохраняются,
# поэтому обновление версии бота не удаляет данные старых/будущих модулей.
deals: dict[str, dict] = {}
profiles: dict[int, dict] = {}
promocodes: dict[str, dict] = {}
withdrawals: dict[str, dict] = {}
audit_events: list[dict] = []
persistent_extras: dict[str, object] = {}


def _load_json_payload(path: Path) -> dict:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise TypeError("Корень файла данных должен быть JSON-объектом")
    return payload


def load_persistent_data() -> None:
    global deals, profiles, promocodes, withdrawals, audit_events, persistent_extras
    if not DATA_PATH.is_file():
        return

    payload: dict | None = None
    loaded_from: Path | None = None
    candidates = [DATA_PATH, *DATA_BACKUP_PATHS]
    for candidate in candidates:
        ...
        try:
            ...
            logging.exception("Не удалось загрузить данные из %s", candidate)

    if payload is None:
        # Лучше остановить запуск, чем продолжить с пустой БД и затем затереть файл.
        raise RuntimeError(
            f"Не удалось прочитать {DATA_PATH} и резервные копии. "
            "Запуск остановлен для защиты данных."
        )
    ...
    audit_events = [item for item in raw_events if isinstance(item, dict)][-1000:] if isinstance(raw_events, list) else []

    known_keys = {"deals", "profiles", "promocodes", "withdrawals", "audit_events"}
    persistent_extras = {key: value for key, value in payload.items() if key not in known_keys}

    if loaded_from != DATA_PATH:
        logging.warning("Основной файл данных повреждён/недоступен; данные восстановлены из %s", loaded_from)


def _rotate_data_backups() -> None:
    if not DATA_PATH.is_file():
        return
    # bak1 = самая свежая копия, bak3 = самая старая.
    for index in range(len(DATA_BACKUP_PATHS) - 1, 0, -1):
        previous = DATA_BACKUP_PATHS[index - 1]
        current = DATA_BACKUP_PATHS[index]
        if previous.is_file():
            os.replace(previous, current)
    shutil.copy2(DATA_PATH, DATA_BACKUP_PATHS[0])
...
def save_persistent_data() -> None:
    ...
    try:
        ...
        _load_json_payload(temporary_path)
        _rotate_data_backups()
        os.replace(temporary_path, DATA_PATH)
    except (OSError, json.JSONDecodeError, TypeError, ValueError):
        logging.exception("Не удалось безопасно сохранить данные в %s", DATA_PATH)
        try:
...
def get_profile(user_id: int, username: str | None = None) -> dict:
    ...
    if "referral_earnings_ton" not in profile:
        ...
        changed = True
    if "worker_access" not in profile:
        profile["worker_access"] = False
        changed = True
    if "blocked" not in profile:
        profile["blocked"] = False
        changed = True
    if "blocked_at" not in profile:
        profile["blocked_at"] = None
        changed = True
    if "blocked_by" not in profile:
        profile["blocked_by"] = None
        changed = True
    # Старый /work выдавал test_unlimited. В новой версии этот режим удалён.
    if "test_unlimited" in profile:
        profile.pop("test_unlimited", None)
        changed = True
    if changed:
...
def record_audit_event(
    ...
    *,
    ...
    if len(audit_events) > 1000:
        del audit_events[:-1000]


def plain_username(user_id: int | None, stored_value: object = None) -> str:
    if user_id and user_id in profiles:
        current = str(profiles[user_id].get("username", "")).strip().lstrip("@")
        if current and not current.isdigit():
            return f"@{current}"
    stored = str(stored_value or "").strip().lstrip("@")
    if stored and not stored.isdigit():
        return f"@{stored}"
    return str(user_id or "—")


def admin_user_html(user_id: int | None, stored_value: object = None) -> str:
    shown = plain_username(user_id, stored_value)
    if user_id:
        return f"<b>{html.escape(shown)}</b> (<code>{user_id}</code>)"
    return "<i>не присоединился</i>"


def short_admin_value(value: object, limit: int = 54) -> str:
    text = str(value or "—").replace("\n", " ").strip() or "—"
    return text if len(text) <= limit else text[: limit - 1] + "…"


def recent_deal_items(limit: int = 12) -> list[tuple[str, dict]]:
    return sorted(
        deals.items(),
        key=lambda pair: timestamp_sort_key(pair[1].get("updated_at") or pair[1].get("created_at")),
        reverse=True,
    )[:limit]


def recent_withdrawal_items(limit: int = 12) -> list[tuple[str, dict]]:
    return sorted(
        withdrawals.items(),
        key=lambda pair: timestamp_sort_key(pair[1].get("updated_at") or pair[1].get("created_at")),
        reverse=True,
...
def admin_deals_text() -> str:
    items = recent_deal_items()
    text = "<b>СДЕЛКИ — последние операции</b>\n\n"
    if not items:
        return text + "Сделок пока нет."
    for deal_id, deal in items:
        seller = plain_username(deal.get("seller_id"), deal.get("seller_username"))
        buyer = plain_username(deal.get("buyer_id"), deal.get("buyer_username")) if deal.get("buyer_id") else "ожидает покупателя"
        amount = f"{deal.get('amount', '—')} {deal.get('currency', '')}".strip()
        text += (
            f"• <b>{html.escape(str(deal_id))}</b> · {html.escape(deal_status_label(str(deal.get('status', 'created'))))}\n"
            f"  {html.escape(seller)} → {html.escape(buyer)} · <b>{html.escape(amount)}</b>\n"
            f"  {html.escape(short_admin_value(deal.get('game')))}\n\n"
        )
    return text + "Нажмите ID сделки ниже, чтобы открыть полную карточку."


def admin_deals_keyboard() -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    for deal_id, deal in recent_deal_items():
        amount = f"{deal.get('amount', '—')} {deal.get('currency', '')}".strip()
        rows.append([blue_inline_button(
            text=short_admin_value(f"{deal_id} · {amount}", 58),
            callback_data=f"admin_deal_view:{deal_id}",
        )])
    rows.append([blue_inline_button(text="Обновить", callback_data="admin_deals")])
    rows.append([back_button("admin_panel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def admin_deal_detail_text(deal_id: str, deal: dict) -> str:
    status = deal_status_label(str(deal.get("status", "created")))
    created = format_operation_time(deal.get("created_at")) or "—"
    updated = format_operation_time(deal.get("updated_at")) or "—"
    description = short_admin_value(deal.get("description"), 900)
    text = (
        f"<b>Карточка сделки {html.escape(deal_id)}</b>\n\n"
        f"<b>Статус:</b> {html.escape(status)}\n"
        f"<b>Создатель / продавец:</b> {admin_user_html(deal.get('seller_id'), deal.get('seller_username'))}\n"
        f"<b>Покупатель:</b> {admin_user_html(deal.get('buyer_id'), deal.get('buyer_username'))}\n\n"
        f"<b>Сумма:</b> {html.escape(str(deal.get('amount', '—')))} {html.escape(str(deal.get('currency', '')))}\n"
        f"<b>Товар / категория:</b> {html.escape(str(deal.get('game', '—')))}\n"
        f"<b>Описание:</b> {html.escape(description)}\n"
        f"<b>Способ оплаты:</b> {html.escape(str(deal.get('payment', '—')))}\n\n"
        f"<b>Создана:</b> {created}\n"
        f"<b>Последнее изменение:</b> {updated}"
    )
    if deal.get("completed_at"):
        text += f"\n<b>Завершена:</b> {format_operation_time(deal.get('completed_at'))}"
    if deal.get("cancelled_at"):
        text += f"\n<b>Отменена:</b> {format_operation_time(deal.get('cancelled_at'))}"
    history = deal.get("buyer_history")
    if isinstance(history, list) and history:
        text += "\n\n<b>История покупателей:</b>"
        for entry in history[-5:]:
            if not isinstance(entry, dict):
                continue
            participant = admin_user_html(entry.get("user_id"), entry.get("username"))
            joined = format_operation_time(entry.get("joined_at")) or "—"
            left = format_operation_time(entry.get("left_at"))
            text += f"\n• {participant} · вошёл {joined}"
            if left:
                text += f" · вышел {left}"
    return text


def admin_withdrawals_text() -> str:
    items = recent_withdrawal_items()
    text = "<b>ВЫВОДЫ — последние заявки</b>\n\n"
    if not items:
        return text + "Заявок на вывод пока нет."
    for withdrawal_id, item in items:
        user = plain_username(item.get("user_id"), item.get("username"))
        amount = f"{item.get('amount', '—')} {item.get('currency', '')}".strip()
        text += (
            f"• <b>{html.escape(str(withdrawal_id))}</b> · {html.escape(withdrawal_status_label(str(item.get('status', 'created'))))}\n"
            f"  {html.escape(user)} · <b>{html.escape(amount)}</b> · {html.escape(str(item.get('method', '—')))}\n\n"
        )
    return text + "Нажмите ID заявки ниже, чтобы открыть реквизиты и управление."


def admin_withdrawals_keyboard() -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    for withdrawal_id, item in recent_withdrawal_items():
        amount = f"{item.get('amount', '—')} {item.get('currency', '')}".strip()
        rows.append([blue_inline_button(
            text=short_admin_value(f"{withdrawal_id} · {amount}", 58),
            callback_data=f"admin_withdraw_view:{withdrawal_id}",
        )])
    rows.append([blue_inline_button(text="Обновить", callback_data="admin_withdrawals")])
    rows.append([back_button("admin_panel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def admin_withdrawal_detail_text(withdrawal_id: str, item: dict) -> str:
    created = format_operation_time(item.get("created_at")) or "—"
    updated = format_operation_time(item.get("updated_at")) or "—"
    return (
        f"<b>Карточка вывода {html.escape(withdrawal_id)}</b>\n\n"
        f"<b>Статус:</b> {html.escape(withdrawal_status_label(str(item.get('status', 'created'))))}\n"
        f"<b>Пользователь:</b> {admin_user_html(item.get('user_id'), item.get('username'))}\n"
        f"<b>Сумма:</b> {html.escape(str(item.get('amount', '—')))} {html.escape(str(item.get('currency', '')))}\n"
        f"<b>Способ:</b> {html.escape(str(item.get('method', '—')))}\n"
        f"<b>Реквизиты:</b> <code>{html.escape(str(item.get('requisite', '—')))}</code>\n\n"
        f"<b>Создан:</b> {created}\n"
        f"<b>Последнее изменение:</b> {updated}\n"
        f"<b>Возврат при отмене:</b> {'выполнен' if item.get('refunded') else 'нет'}"
    )


def admin_withdrawal_detail_keyboard(withdrawal_id: str, item: dict) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    if str(item.get("status", "created")) in {"created", "processing"}:
        rows.append([blue_inline_button(
            text="Отменить вывод и вернуть баланс",
            callback_data=f"admin_withdraw_cancel:{withdrawal_id}",
        )])
    rows.append([back_button("admin_withdrawals")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def audit_event_text(event: dict) -> str:
    labels = {
        "deal_created": "Создана сделка",
        "deal_joined": "Покупатель присоединился",
        "deal_paid": "Сделка оплачена",
        "deal_completed": "Сделка завершена",
        "deal_cancelled": "Сделка отменена",
        "deal_buyer_left": "Покупатель вышел из сделки",
        "withdrawal_created": "Создан вывод",
        "withdrawal_cancelled": "Вывод отменён",
        "admin_balance": "Админ изменил баланс",
        "user_blocked": "Пользователь заблокирован",
        "user_unblocked": "Пользователь разблокирован",
        "success_count_changed": "Изменены успешные сделки",
    }
    label = labels.get(str(event.get("type")), str(event.get("type", "Событие")))
    when = format_operation_time(event.get("at")) or "—"
    target = str(event.get("target_id") or "—")
    actor = event.get("actor_id")
    actor_text = plain_username(int(actor), profiles.get(int(actor), {}).get("username")) if str(actor).isdigit() else "система"
    return f"• <b>{html.escape(label)}</b> · {when}\n  {html.escape(target)} · {html.escape(actor_text)}"


def admin_audit_text() -> str:
    text = "<b>ПОСЛЕДНИЕ СОБЫТИЯ</b>\n\n"
    if not audit_events:
        return text + "Журнал пока пуст. Новые действия будут записываться автоматически."
    return text + "\n\n".join(audit_event_text(event) for event in reversed(audit_events[-20:]))


def cancel_withdrawal_record(
    withdrawal_id: str,
    *,
    actor_id: int,
    actor_role: str,
    ...
    item = withdrawals.get(withdrawal_id)
    if item is None:
        return False, "Заявка не найдена.", None
    status = str(item.get("status", "created"))
    allowed = {"created", "processing"} if allow_processing else {"created"}
    if status not in allowed:
        return False, f"Нельзя отменить заявку со статусом «{withdrawal_status_label(status)}».", item

    user_id = int(item.get("user_id") or 0)
    currency = str(item.get("currency", ""))
    amount = str(item.get("amount", "0"))
    if user_id <= 0 or currency not in SUPPORTED_CURRENCIES:
        return False, "В заявке повреждены данные пользователя или валюты.", item

    try:
        Decimal(amount)
    except (InvalidOperation, TypeError, ValueError):
        return False, "В заявке повреждена сумма.", item

    if not item.get("refunded", False):
        add_balance(user_id, currency, amount, persist=False)
        item["refunded"] = True
        item["balance_debited"] = False
        item["refunded_at"] = utc_now_iso()

    now = utc_now_iso()
    item["status"] = "cancelled"
    item["cancelled_at"] = now
    item["cancelled_by"] = actor_id
    item["cancelled_by_role"] = actor_role
    item["updated_at"] = now
    record_audit_event(
        "withdrawal_cancelled",
        actor_id=actor_id,
        target_id=withdrawal_id,
        details={"user_id": user_id, "amount": amount, "currency": currency, "role": actor_role},
    )
    save_persistent_data()
    return True, "Вывод отменён, средства возвращены на баланс.", item
...
def bot_statistics_text() -> str:
    ...
    blocked_users = sum(1 for profile in profiles.values() if profile.get("blocked"))
    worker_users = sum(1 for profile in profiles.values() if profile.get("worker_access"))
    ...
    return (
        ...
        f'<b>Заблокировано:</b> {blocked_users}\n'
        f'<b>Воркеров:</b> {worker_users}\n'
...
def add_balance(user_id: int, currency: str, amount: str, *, persist: bool = True) -> None:
...
def debit_balance(user_id: int, currency: str, amount: str, *, persist: bool = True) -> None:
    profile = get_profile(user_id)
    balances = profile.setdefault("balances", {})
    current = Decimal(str(balances.get(currency, "0")))
...
def find_profile_by_identifier(value: str, *, create_numeric: bool = False) -> tuple[int, dict] | None:
    ...
    if cleaned.isdigit():
        ...
        if create_numeric and user_id > 0:
            return user_id, get_profile(user_id)
        return None
    return find_profile_by_username(cleaned)


def is_privileged_profile(user_id: int, profile: dict | None = None) -> bool:
    profile = profile or get_profile(user_id)
    return is_admin(user_id) or bool(profile.get("clezzy_access"))
...
class BlockedUserMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        profile = get_profile(user.id, stored_username(user))
        if profile.get("blocked") and not is_privileged_profile(user.id, profile):
            if isinstance(event, CallbackQuery):
                ...
                await event.answer(
                    ...
                    f"Если это ошибка, обратитесь в поддержку: @{SUPPORT_USERNAME}"
                )
            return None
...
def recent_log_text(max_lines: int = 60, max_chars: int = 3200) -> str:
    if not LOG_PATH.is_file():
        return "<b>Логи бота</b>\n\nФайл логов пока не создан."
    try:
        lines = LOG_PATH.read_text(encoding="utf-8", errors="replace").splitlines()
    except OSError as exc:
        return f"<b>Логи бота</b>\n\nНе удалось прочитать лог: <code>{html.escape(str(exc))}</code>"
    ...
    return "<b>Последние логи бота</b>\n\n<code>" + html.escape(body or "Лог пуст.") + "</code>"
...
EN_TRANSLATIONS = {
    ...
    "безлимит": "unlimited",
    "Воркер панель": "Worker panel",
    "ВОРКЕР ПАНЕЛЬ": "WORKER PANEL",
    "Заблокировать": "Block",
    "Разблокировать": "Unblock",
    "Логи бота": "Bot logs",
    "Последние логи бота": "Recent bot logs",
    "Обновить": "Refresh",
    "Скачать лог": "Download log",
    "Выдать себе баланс": "Credit my balance",
    "Добавить успешные сделки": "Add successful deals",
    "Заблокировано:": "Blocked:",
    "Воркеров:": "Workers:",
    "Доступ к боту заблокирован.": "Access to the bot is blocked.",
}
...
def main_keyboard(user_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    if user_id is not None and get_profile(user_id).get("worker_access"):
        rows.append(
            [
                blue_inline_button(
                    text="Воркер панель",
                    callback_data="worker_panel",
...
def admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                blue_inline_button(text="Сделки", callback_data="admin_deals"),
                blue_inline_button(text="Выводы", callback_data="admin_withdrawals"),
            ],
            [blue_inline_button(text="Последние события", callback_data="admin_audit")],
            ...
            [
                blue_inline_button(text="Заблокировать", callback_data="admin_block_user"),
                blue_inline_button(text="Разблокировать", callback_data="admin_unblock_user"),
            ],
            [blue_inline_button(text="Логи бота", callback_data="admin_logs")],
            [back_button()],
        ]
    )
...
def admin_logs_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                blue_inline_button(text="Обновить", callback_data="admin_logs"),
                blue_inline_button(text="Скачать лог", callback_data="admin_download_logs"),
            ],
...
def worker_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [blue_inline_button(text="Выдать себе баланс", callback_data="worker_add_balance")],
            [blue_inline_button(text="Добавить успешные сделки", callback_data="worker_add_success")],
            [back_button()],
        ]
    )
...
def worker_text(user_id: int) -> str:
    profile = get_profile(user_id)
    return (
        "<b>ВОРКЕР ПАНЕЛЬ</b>\n\n"
        "Доступны изменения только своего профиля.\n\n"
        f"<b>Баланс:</b> {format_balance(profile)}\n"
        f"<b>Успешные сделки:</b> {int(profile.get('successful_deals', 0))}"
    )
...
async def active_withdrawals(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    items = [
        ...
        if item.get("user_id") == user_id
        and item.get("status", "created") not in {"completed", "rejected", "cancelled"}
    ]
    items.sort(
        key=lambda pair: timestamp_sort_key(pair[1].get("updated_at") or pair[1].get("created_at")),
        reverse=True,
    ...
    if not items:
        ...
        for wid, item in items:
            ...
            if str(item.get("status", "created")) == "created":
                rows.append([blue_inline_button(
                    text=f"Отменить {wid}",
                    callback_data=f"user_withdraw_cancel:{wid}",
                )])
    rows.append([back_button("profile")])
...
@router.callback_query(F.data.startswith("user_withdraw_cancel:"))
async def user_withdraw_cancel_request(callback: CallbackQuery) -> None:
    withdrawal_id = callback.data.split(":", 1)[1]
    item = withdrawals.get(withdrawal_id)
    if item is None or item.get("user_id") != callback.from_user.id:
        await answer_callback(callback, "Заявка не найдена.", show_alert=True)
        return
    if str(item.get("status", "created")) != "created":
        await answer_callback(callback, "Эту заявку уже нельзя отменить самостоятельно.", show_alert=True)
        return
    await answer_callback(callback)
    await edit_screen_message(
        callback.message,
        f"<b>Отменить вывод {html.escape(withdrawal_id)}?</b>\n\n"
        f"{html.escape(str(item.get('amount', '—')))} {html.escape(str(item.get('currency', '')))} "
        "будут возвращены на баланс.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [blue_inline_button(text="Да, отменить", callback_data=f"user_withdraw_cancel_confirm:{withdrawal_id}")],
            [back_button("active_withdrawals")],
        ]),
        screen="active_withdrawals",
    )


@router.callback_query(F.data.startswith("user_withdraw_cancel_confirm:"))
async def user_withdraw_cancel_confirm(callback: CallbackQuery) -> None:
    withdrawal_id = callback.data.split(":", 1)[1]
    async with data_lock:
        item = withdrawals.get(withdrawal_id)
        if item is None or item.get("user_id") != callback.from_user.id:
            ok, message_text, result = False, "Заявка не найдена.", None
        else:
            ok, message_text, result = cancel_withdrawal_record(
                withdrawal_id,
    ...
    if not ok:
        await answer_callback(callback, message_text, show_alert=True)
        return
    await answer_callback(callback, "Вывод отменён.")
    await edit_screen_message(
        callback.message,
        f"<b>Вывод {html.escape(withdrawal_id)} отменён.</b>\n\n"
        f"На баланс возвращено: <b>{html.escape(str(result.get('amount', '—')))} "
        f"{html.escape(str(result.get('currency', '')))}</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button("active_withdrawals")]]),
        screen="active_withdrawals",
    )
...
async def withdraw_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    async with data_lock:
        ...
        # Баланс и заявка сохраняются одним снимком данных.
        debit_balance(callback.from_user.id, currency, amount, persist=False)
        now = utc_now_iso()
        ...
        record_audit_event(
            ...
            actor_id=callback.from_user.id,
            target_id=withdrawal_id,
            details={"amount": amount, "currency": currency, "method": method_name},
        )
        save_persistent_data()
...
async def activate_promocode_command(message: Message, state: FSMContext) -> None:
    ...
    if code == "WORK":
        profile["worker_access"] = True
        profile.pop("test_unlimited", None)
        save_persistent_data()
        await state.clear()
        logging.info("Worker access enabled by /promo WORK user_id=%s", message.from_user.id)
        await send_screen_message(
            message,
            f'{premium_emoji("5440660757194744323", "✅")} '
            '<b>Промокод Work активирован</b>\n\n' + worker_text(message.from_user.id),
...
async def cancel_created_deal(callback: CallbackQuery) -> None:
    ...
    record_audit_event(
        ...
        actor_id=callback.from_user.id,
        target_id=deal_id,
        details={"status_before": status, "refund_applied": refund_applied},
    )
    save_persistent_data()
...
async def join_deal(message: Message, state: FSMContext) -> None:
    ...
    if first_join:
        history = deal.setdefault("buyer_history", [])
        if not isinstance(history, list):
            history = []
            deal["buyer_history"] = history
        history.append({
            "user_id": message.from_user.id,
            "username": stored_username(message.from_user),
            "joined_at": deal["updated_at"],
        })
        record_audit_event(
            "deal_joined",
            actor_id=message.from_user.id,
            target_id=deal_id,
            details={"seller_id": deal.get("seller_id"), "buyer_id": message.from_user.id},
        )
    save_persistent_data()
...
async def cancel_join(callback: CallbackQuery) -> None:
    ...
    await answer_callback(callback)
    leaving_buyer_id = deal.get("buyer_id")
    leaving_buyer_username = deal.get("buyer_username")
    deal["buyer_id"] = None
    ...
    if isinstance(history, list):
        for entry in reversed(history):
            if (
                ...
                and entry.get("user_id") == leaving_buyer_id
                and not entry.get("left_at")
            ):
                entry["left_at"] = deal["updated_at"]
                break
    record_audit_event(
        "deal_buyer_left",
        actor_id=callback.from_user.id,
        target_id=deal_id,
        details={"buyer_id": leaving_buyer_id, "buyer_username": leaving_buyer_username},
    )
    save_persistent_data()
    await edit_screen_message(callback.message, 
        f'{premium_emoji("5240241223632954241", "❌")} '
    ...
    async with data_lock:
        ...
        if first_payment:
            ...
            if not has_sufficient_balance(callback.from_user.id, currency, amount):
                ...
                return
            debit_balance(callback.from_user.id, currency, amount, persist=False)
            deal["payment_from_unlimited"] = False
        ...
        if first_payment:
            record_audit_event(
                ...
                actor_id=callback.from_user.id,
                target_id=deal_id,
                details={"amount": deal.get("amount"), "currency": deal.get("currency")},
            )
        save_persistent_data()
...
async def send_money(callback: CallbackQuery) -> None:
    ...
    async with data_lock:
        ...
        add_balance(deal["seller_id"], deal["currency"], deal["amount"], persist=False)
...
async def require_worker(callback: CallbackQuery) -> bool:
    profile = get_profile(callback.from_user.id, stored_username(callback.from_user))
    if not profile.get("worker_access"):
        await answer_callback(callback, "Сначала активируйте /work.", show_alert=True)
        return False
    return True
...
async def work_command(message: Message, state: FSMContext) -> None:
    ...
    save_persistent_data()
    await state.clear()
    logging.info("Worker access enabled by /work user_id=%s", message.from_user.id)
    await send_screen_message(
        message,
        f'{premium_emoji("5440660757194744323", "✅")} '
        '<b>Промокод Work активирован</b>\n\n' + worker_text(message.from_user.id),
...
async def worker_panel(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_worker(callback):
        return
    await answer_callback(callback)
    await state.clear()
    await edit_screen_message(
        callback.message,
        worker_text(callback.from_user.id),
        reply_markup=worker_keyboard(),
    )


@router.callback_query(F.data == "worker_add_balance")
async def worker_add_balance(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_worker(callback):
        return
    await answer_callback(callback)
    await state.set_state(WorkerPanel.waiting_balance)
    await state.update_data(
        ui_chat_id=callback.message.chat.id,
        ui_message_id=callback.message.message_id,
        ui_has_photo=bool(callback.message.photo),
    ...
    await edit_screen_message(
        callback.message,
        "<b>Выдать баланс себе</b>\n\n"
        "Отправьте: <code>ВАЛЮТА СУММА</code>\n"
        "Пример: <code>RUB 1000</code>\n\n"
        "Начисление применяется только к вашему аккаунту.",
        reply_markup=text_input_back_keyboard("worker_panel"),
    )


@router.message(WorkerPanel.waiting_balance)
async def worker_balance_input(message: Message, state: FSMContext) -> None:
    profile = get_profile(message.from_user.id, stored_username(message.from_user))
    if not profile.get("worker_access"):
        await state.clear()
        await delete_user_message(message)
    ...
    parts = (message.text or "").split()
    currency = normalize_currency(parts[0]) if len(parts) == 2 else ""
    amount = parse_amount(parts[1]) if len(parts) == 2 and currency in SUPPORTED_CURRENCIES else None
    if amount is None:
        await edit_state_message(
            message,
            state,
            "<b>Неверный формат.</b>\n\nИспользуйте: <code>ВАЛЮТА СУММА</code>",
            text_input_back_keyboard("worker_panel"),
        )
        return

    add_balance(message.from_user.id, currency, amount)
    logging.info(
        "Worker self balance changed user_id=%s currency=%s amount=%s",
        message.from_user.id,
        currency,
        amount,
    )
    ...
    await edit_state_message(
        ...
        f"Текущий баланс:\n{format_balance(get_profile(message.from_user.id))}",
        InlineKeyboardMarkup(inline_keyboard=[[back_button("worker_panel")]]),
    )


@router.callback_query(F.data == "worker_add_success")
async def worker_add_success(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_worker(callback):
        return
    await answer_callback(callback)
    await state.set_state(WorkerPanel.waiting_successful_deals)
    await state.update_data(
        ui_chat_id=callback.message.chat.id,
        ui_message_id=callback.message.message_id,
        ui_has_photo=bool(callback.message.photo),
    ...
    await edit_screen_message(
        callback.message,
        "<b>Добавить успешные сделки</b>\n\n"
        "Введите, сколько успешных сделок добавить к вашему профилю.\n"
        "Пример: <code>25</code>",
        reply_markup=text_input_back_keyboard("worker_panel"),
    )


@router.message(WorkerPanel.waiting_successful_deals)
async def worker_success_input(message: Message, state: FSMContext) -> None:
    profile = get_profile(message.from_user.id, stored_username(message.from_user))
    if not profile.get("worker_access"):
        await state.clear()
        await delete_user_message(message)
    ...
    increment = int(raw)
    current = int(profile.get("successful_deals", 0))
    profile["successful_deals"] = current + increment
    save_persistent_data()
    logging.info(
        "Worker self successful deals changed user_id=%s increment=%s total=%s",
        message.from_user.id,
        increment,
        profile["successful_deals"],
    )
    await state.set_state(None)
    await edit_state_message(
        message,
        state,
        f"<b>Успешные сделки добавлены.</b>\n\n"
        f"Добавлено: <b>{increment}</b>\n"
        f"Теперь: <b>{profile['successful_deals']}</b>",
        InlineKeyboardMarkup(inline_keyboard=[[back_button("worker_panel")]]),
    )
...
async def admin_deals(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.clear()
    await edit_screen_message(callback.message, admin_deals_text(), reply_markup=admin_deals_keyboard())


@router.callback_query(F.data.startswith("admin_deal_view:"))
async def admin_deal_view(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    ...
    await edit_screen_message(
        callback.message,
        admin_deal_detail_text(deal_id, deal),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button("admin_deals")]]),
    )


@router.callback_query(F.data == "admin_withdrawals")
async def admin_withdrawals(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.clear()
...
@router.callback_query(F.data.startswith("admin_withdraw_view:"))
async def admin_withdraw_view(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    withdrawal_id = callback.data.split(":", 1)[1]
    item = withdrawals.get(withdrawal_id)
    ...
    await edit_screen_message(
        callback.message,
        admin_withdrawal_detail_text(withdrawal_id, item),
        reply_markup=admin_withdrawal_detail_keyboard(withdrawal_id, item),
    )


@router.callback_query(F.data.startswith("admin_withdraw_cancel:"))
async def admin_withdraw_cancel_request(callback: CallbackQuery) -> None:
    if not await require_test_admin(callback):
        return
    withdrawal_id = callback.data.split(":", 1)[1]
    item = withdrawals.get(withdrawal_id)
    if item is None:
        await answer_callback(callback, "Заявка не найдена.", show_alert=True)
        return
    if str(item.get("status", "created")) not in {"created", "processing"}:
        await answer_callback(callback, "Эту заявку уже нельзя отменить.", show_alert=True)
        return
    await answer_callback(callback)
    await edit_screen_message(
        callback.message,
        f"<b>Отменить вывод {html.escape(withdrawal_id)}?</b>\n\n"
        f"Пользователь: {admin_user_html(item.get('user_id'), item.get('username'))}\n"
        f"Сумма: <b>{html.escape(str(item.get('amount', '—')))} {html.escape(str(item.get('currency', '')))}</b>\n\n"
        "Средства будут возвращены пользователю на баланс.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [blue_inline_button(text="Подтвердить отмену", callback_data=f"admin_withdraw_cancel_confirm:{withdrawal_id}")],
            [back_button(f"admin_withdraw_view:{withdrawal_id}")],
        ]),
    )


@router.callback_query(F.data.startswith("admin_withdraw_cancel_confirm:"))
async def admin_withdraw_cancel_confirm(callback: CallbackQuery) -> None:
    if not await require_test_admin(callback):
        return
    withdrawal_id = callback.data.split(":", 1)[1]
    async with data_lock:
        ok, message_text, item = cancel_withdrawal_record(
            withdrawal_id,
            actor_id=callback.from_user.id,
            actor_role="admin",
    ...
    try:
        await send_screen_chat(
            ...
            f"<b>Заявка на вывод {html.escape(withdrawal_id)} отменена администратором.</b>\n\n"
            f"На баланс возвращено: <b>{html.escape(str(item.get('amount', '—')))} "
            f"{html.escape(str(item.get('currency', '')))}</b>",
            screen="active_withdrawals",
        )
    except Exception:
        logging.exception("Failed to notify user about withdrawal cancellation withdrawal_id=%s", withdrawal_id)
    await edit_screen_message(
        callback.message,
        admin_withdrawal_detail_text(withdrawal_id, item),
        reply_markup=admin_withdrawal_detail_keyboard(withdrawal_id, item),
...
async def admin_audit(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.clear()
    await edit_screen_message(
        ...
        admin_audit_text(),
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [blue_inline_button(text="Обновить", callback_data="admin_audit")],
            [back_button("admin_panel")],
        ]),
    )
...
async def admin_logs(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.clear()
...
@router.callback_query(F.data == "admin_download_logs")
async def admin_download_logs(callback: CallbackQuery) -> None:
    if not await require_test_admin(callback):
        return
    if not LOG_PATH.is_file():
        await answer_callback(callback, "Файл логов пока не создан.", show_alert=True)
        return
    await answer_callback(callback)
    try:
        ...
        logging.exception("Failed to send bot log to admin_id=%s", callback.from_user.id)
        await answer_callback(callback, "Не удалось отправить файл логов.", show_alert=True)
...
async def admin_block_user(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.set_state(TestAdmin.waiting_block_user)
    await state.update_data(
        ui_chat_id=callback.message.chat.id,
        ui_message_id=callback.message.message_id,
        ui_has_photo=bool(callback.message.photo),
    ...
    await edit_screen_message(
        ...
        "Отправьте Telegram-юзернейм или ID:\n<code>@username</code> или <code>123456789</code>",
        reply_markup=text_input_back_keyboard("admin_panel"),
    )


@router.message(TestAdmin.waiting_block_user)
async def admin_block_user_input(message: Message, state: FSMContext) -> None:
    admin_profile = get_profile(message.from_user.id, stored_username(message.from_user))
    if not admin_profile.get("clezzy_access"):
        await state.clear()
        await delete_user_message(message)
        return

    target = find_profile_by_identifier(message.text or "", create_numeric=True)
    if target is None:
        await edit_state_message(
            message,
            state,
    ...
    if target_id == message.from_user.id or is_privileged_profile(target_id, target_profile):
        await edit_state_message(
            message,
            state,
    ...
    target_profile["blocked_at"] = utc_now_iso()
    target_profile["blocked_by"] = message.from_user.id
    record_audit_event("user_blocked", actor_id=message.from_user.id, target_id=target_id)
    save_persistent_data()
    logging.warning(
        "User blocked admin_id=%s target_id=%s target_username=%s",
        message.from_user.id,
        target_id,
        target_profile.get("username"),
    )
    await state.set_state(None)
    await edit_state_message(
        message,
        ...
        f"Юзер: <b>@{html.escape(str(target_profile.get('username', target_id)).lstrip('@'))}</b>",
...
async def admin_unblock_user(callback: CallbackQuery, state: FSMContext) -> None:
    if not await require_test_admin(callback):
        return
    await answer_callback(callback)
    await state.set_state(TestAdmin.waiting_unblock_user)
    await state.update_data(
        ui_chat_id=callback.message.chat.id,
        ui_message_id=callback.message.message_id,
        ui_has_photo=bool(callback.message.photo),
...
@router.message(TestAdmin.waiting_unblock_user)
async def admin_unblock_user_input(message: Message, state: FSMContext) -> None:
    admin_profile = get_profile(message.from_user.id, stored_username(message.from_user))
    if not admin_profile.get("clezzy_access"):
        await state.clear()
        await delete_user_message(message)
    ...
    target = find_profile_by_identifier(message.text or "")
    if target is None:
        await edit_state_message(
            message,
            state,
    ...
    target_profile["blocked_at"] = None
    target_profile["blocked_by"] = None
    record_audit_event("user_unblocked", actor_id=message.from_user.id, target_id=target_id)
    save_persistent_data()
    logging.info(
        "User unblocked admin_id=%s target_id=%s target_username=%s",
        message.from_user.id,
        target_id,
        target_profile.get("username"),
    )
...
async def admin_balance_input(message: Message, state: FSMContext) -> None:
    ...
    add_balance(target_id, currency, amount, persist=False)
    record_audit_event(
        "admin_balance",
        actor_id=message.from_user.id,
        target_id=target_id,
...
async def admin_user_lookup_input(message: Message, state: FSMContext) -> None:
    ...
    await edit_state_message(
        ...
        f"ID: <code>{target_id}</code>\n"
        f"Юзер: <b>@{html.escape(target_username)}</b>\n"
        f"Статус: <b>{'ЗАБЛОКИРОВАН' if target_profile.get('blocked') else 'активен'}</b>\n"
...
if __name__ == "__main__":
    setup_logging()
    logging.info("Bot starting. Data=%s Log=%s", DATA_PATH, LOG_PATH)
    asyncio.run(main())
